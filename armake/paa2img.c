...
int paa2img(char *source, Image *output) {
    ...
    while (true) {
        ...
        if (strcmp(taggsig, "GGAT")) {
            fprintf(stderr, "Failed to find MIPMAP pointer.\n");
            fclose(f);
            return 2;
        }
    ...
    if (compression == COMP_LZO) {
        ...
        if (lzo_init() != LZO_E_OK) {
            fprintf(stderr, "Failed to initialize LZO for decompression.\n");
            return 3;
	}
        if (lzo1x_decompress(compresseddata, datalen, imgdata, &out_len, NULL) != LZO_E_OK) {
            fprintf(stderr, "Failed to decompress LZO data.\n");
            return 3;
        }
    } else if (compression == COMP_LZSS) {
        fprintf(stderr, "LZSS compression support is not implemented.\n");
        return 3;
    } else {
    ...
    switch (paatype) {
        case DXT1:
            if (dxt12img(imgdata, outputdata, width, height)) {
                fprintf(stderr, "DXT1 decoding failed.\n");
                return 4;
            }
            break;
        case DXT3:
            fprintf(stderr, "DXT3 support is not implemented.\n");
            return 4;
        case DXT5:
            if (dxt52img(imgdata, outputdata, width, height)) {
                fprintf(stderr, "DXT5 decoding failed.\n");
                return 4;
            }
            break;
        case ARGB4444:
            fprintf(stderr, "ARGB4444 support is not implemented.\n");
            return 4;
        case ARGB1555:
            fprintf(stderr, "ARGB1555 support is not implemented.\n");
            return 4;
        case AI88:
            fprintf(stderr, "GRAY / AI88 support is not implemented.\n");
            return 4;
        default:
            fprintf(stderr, "Unrecognized PAA type.\n");
            return 4;
    }
